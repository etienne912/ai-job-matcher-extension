...
async function callOpenAI(systemPrompt, staticPrompt, dynamicPrompt, userPrompt, apiKey, model, fileData) {
  ...
  if (fileData) {
    // OpenAI supports vision but for PDFs it usually requires Assistants API or specific integrations.
    // However, GPT-4o supports images. For PDF/Docx, we might need a workaround or if the user is using a model that supports it.
    // For now, we'll assume the model might support it if we send it as a data URL in a text prompt, 
    // or just inform that it might not work for raw files without vision.
    // Actually, OpenAI doesn't support PDF in Chat Completions directly yet (only images).
    // Let's try to send it as a message anyway or throw if not image.
    if (fileData.mimeType.startsWith('image/')) {
      ...
      // For PDF/DOCX on OpenAI, we'd normally use Assistants API. 
      // As a fallback, we'll tell the user.
      throw new Error("OpenAI's Chat API doesn't support direct PDF/DOCX extraction yet. Please use Anthropic or Gemini.");
    }
...
async function callMistral(systemPrompt, staticPrompt, dynamicPrompt, userPrompt, apiKey, model, fileData) {
  ...
  if (fileData) {
    throw new Error("Mistral AI doesn't support direct PDF/DOCX extraction via this API yet. Please use Anthropic or Gemini.");
  } else if (staticPrompt && dynamicPrompt) {
    messages.push({ role: 'user', content: staticPrompt });
    messages.push({ role: 'user', content: dynamicPrompt });
  } else {
